		...
		switch(row.type){
			...
			case "popup":
				var recruitPeople = '<tr><td class="from_title">'+row.title+'：</td>'
                    		...
                    		+'errormsg="'+row.errormsg+'" /><div class="rp_view" name="'+row.data.name+'" title="'+(row.data.title ? row.data.title : "")+'" url="'+row.data.url+'" type="'+row.data.type+'"  ><div class="'+row.data.name+'"></div></div></td>'
                    		+'<td><div class="Validform_checktip">'+row.errormsg+'</div>'
                    		+'<div class="info">'+row.tip+'<span class="dec"><s class="dec1">&#9670;</s><s class="dec2">&#9670;</s></span></div></td></tr>';     		
                views += recruitPeople;
			break;
